...
func TestAddDeleteUser(t *testing.T) {
	...
	/*
		log := logrus.New()
		directory, err := merchant.NewDirectory(log, startDatabase(t, log))
		if err != nil {
		...
		t.Run("When deleting an added user", func(t *testing.T) {
			...
			role := merchpb.Role_ADMIN
			user1, err := directory.AddUser(ctx, &merchpb.AddUserRequest{
				Role: role,
				Name: "Foo",
			})
			if err != nil {
			...
			user2, err := directory.DeleteUser(ctx, &merchpb.DeleteUserRequest{
				Id: user1.GetId(),
			})
			if err != nil {
		...
		t.Run("When using a non-uuid in DeleteUser", func(t *testing.T) {
			t.Parallel()
			_, err = directory.DeleteUser(ctx, &merchpb.DeleteUserRequest{
				Id: "not_a_UUID",
			})
...
func TestListUsers(t *testing.T) {
	...
	/*
		...
		t.Cleanup(cancel)
			...
			user2, err := directory.AddUser(ctx, &merchpb.AddUserRequest{
				Role: merchpb.Role_GUEST,
				Name: "Bar",
			})
			if err != nil {
			...
			user3, err := directory.AddUser(ctx, &merchpb.AddUserRequest{
				Role: merchpb.Role_ADMIN,
				Name: "Baz",
			})
			if err != nil {
			...
			t.Run("Returning all users", func(t *testing.T) {
				...
				err := directory.ListUsers(new(merchpb.ListUsersRequest), srv)
				if err != nil {
			...
			t.Run("Filtering by age", func(t *testing.T) {
				...
				err := directory.ListUsers(&merchpb.ListUsersRequest{
		...
		t.Cleanup(cancel)
			...
			t.Run(fmt.Sprintf("Add %d users", numUsers), func(t *testing.T) {
				...
				for i := 0; i < numUsers; i++ {
					addSrv.reqs = append(addSrv.reqs, &merchpb.AddUserRequest{
						Role: merchpb.Role_GUEST,
						Name: "Foo",
					})
				}
				...
				err = directory.ListUsers(new(merchpb.ListUsersRequest), listSrv)
				if err != nil {
					t.Fatalf("Failed to list users: %s", err)
				}
...
func BenchmarkAddUsers(b *testing.B) {
	/*
		...
		directory, err := merchant.NewDirectory(log, startDatabase(b, log))
		if err != nil {
...
// var benchmarkUser *merchpb.User
...
func BenchmarkAddUser(b *testing.B) {
	/*
		...
		b.ResetTimer()
		b.RunParallel(func(pb *testing.PB) {
			var user *merchpb.User
			for pb.Next() {
				user, err = directory.AddUser(ctx, &merchpb.AddUserRequest{
					Role: merchpb.Role_GUEST,
					Name: "Foo",
				})
				if err != nil {
...
func (a *addUsersSrvFake) Recv() (*merchpb.AddUserRequest, error) {
	if len(a.reqs) == 0 {
		return nil, io.EOF
	}
...
func (l *listUsersSrvFake) Send(user *merchpb.User) error {
